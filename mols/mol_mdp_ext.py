...
class BlockMoleculeDataExtended(BlockMoleculeData):
    ...
    def as_dict(self):
        return {'blockidxs': self.blockidxs, 'slices': self.slices, 'numblocks': self.numblocks, 'jbonds': self.jbonds, 'stems': self.stems}
...
class MolMDPExtended(MolMDP):
    ...
    def parents(self, mol=None):
        ...
        for rblockidx in blocks_degree_1:
            ...
            removed_bonds = [(jbidx, bond) for jbidx, bond in enumerate(new_mol.jbonds) if rblockidx in bond[:2]]
            ...
            stem = ([reindex[rbond[0]], rbond[2]] if rblockidx == rbond[1] else [reindex[rbond[1]], rbond[3]])
            # and add it back
            ...
            removed_stem_atom = (rbond[3] if rblockidx == rbond[1] else rbond[2])
    ...
    def post_init(self, device, repr_type, include_bonds=False, include_nblocks=False):
        ...
        self.stem_type_offset = np.int32([0] + list(np.cumsum([max(self.block_rs[self.block_smi.index(i)])+1 for i in self.true_block_set])))
    ...
    def mol2repr(self, mol=None):
        ...
        if self.repr_type == 'block_graph':
            ...
            r = model_atom.mol2graph(
                mol, self, self.floatX, bonds=self.include_bonds, nblocks=self.include_nblocks
            )
        elif self.repr_type == 'morgan_fingerprint':
            r = model_fingerprint.mol2fp(
                mol, self, self.floatX
            )
