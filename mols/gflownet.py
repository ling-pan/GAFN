...
parser.add_argument("--sp_r_thres", default=7., type=float) 

parser.add_argument("--w_ri", default=0, type=int)
parser.add_argument("--ri_coe", default=0.1, type=float)
...
class Dataset:
    ...
    def _get_reward(self, m):
        ...
        assert len(self.train_mols_map) == 0 
        smi = m.smiles
    ...
    def r2r(self, dockscore=None, normscore=None): 
        ...
        sparse_r = transformed_r if normscore > self.sparse_r_threshold else 1e-9
        transformed_r = sparse_r
...
def make_intrinsic_reward_model(args, mdp):
    assert args.repr_type == 'block_graph'

    intrinsic_reward_model = model_block.RND(
        nemb=args.nemb,
        nvec=0,
        num_conv_steps=args.num_conv_steps,
        mdp_cfg=mdp,
...
def train_model_with_proxy(args, model, proxy, dataset, num_steps=None, do_save=True, intrinsic_reward_model=None):
    ...
    if args.w_ri:
        ri_opt = torch.optim.Adam(
            intrinsic_reward_model.parameters(), 
            args.learning_rate, 
            weight_decay=args.weight_decay,
            betas=(args.opt_beta, args.opt_beta2),
            eps=args.opt_epsilon
    ...
    for i in range(num_steps):
        ...
        if args.objective == 'fm':
            ...
            exp_inflow = (torch.zeros((ntransitions,), device=device, dtype=dataset.floatX).index_add_(0, pb, torch.exp(qsa_p))) # pb is the parents' batch index
            ...
            if args.w_ri:
                ris = intrinsic_reward_model.compute_intrinsic_reward(s_copy)
                outflow_plus_r = torch.log(log_reg_c + r + exp_outflow * (1 - d) + ris)
            else:
                # include reward and done multiplier, then take the log (we're guarenteed that r > 0 iff d = 1, so the log always works)
            ...
            if args.w_ri:
                ri_loss = intrinsic_reward_model.compute_loss(s_copy1)
                loss += args.ri_loss_coe * ri_loss
            ...
            train_losses.append((loss.item(), _term_loss.item(), _flow_loss.item(), term_loss.item(), flow_loss.item()))
            ...
            if args.clip_grad > 0:
                ...
                if args.w_ri:
                    torch.nn.utils.clip_grad_value_(intrinsic_reward_model.predictor_network.parameters(), args.clip_grad)
...
def main(args):
    ...
    model = make_model(args, mdp, out_per_mol=1)
    print ('model', model)
    ...
    if args.w_ri:
        intrinsic_reward_model = make_intrinsic_reward_model(args, mdp)
        print ('intrinsic_reward_model', intrinsic_reward_model)
        intrinsic_reward_model.to(args.floatX)
        intrinsic_reward_model.to(device)
    else:
        intrinsic_reward_model = None

    train_model_with_proxy(args, model, proxy, dataset, do_save=True, intrinsic_reward_model=intrinsic_reward_model)
    print('Done.')
